...
export function Cashier({ 
  ...
  const processTransaction = async () => {
    ...
    const result = await createTransaction({
      ...
      cashReceived: paymentMethod === "CASH" ? sanitizedCash : null,
            ...
            <div className="p-4 bg-white border-t border-[#e5e7eb] space-y-4">
              ...
              {paymentMethod === "CASH" ? (
                ...
                <div className="flex items-center gap-3 p-3 bg-[#f9fafb] rounded-lg border border-[#e5e7eb]">
                  <div className="w-8 h-8 bg-[#111827] rounded flex items-center justify-center text-white">
                    <QrCode size={16} />
                  </div>
                  <div className="flex flex-col">
                    <p className="text-xs font-bold text-[#111827]">QRIS manual</p>
                    <p className="text-[10px] text-[#6b7280]">Scan QRIS kasir, lalu finalisasi di sini</p>
                  </div>
                </div>
              )}
          ...
          <div className="w-full max-w-sm overflow-hidden bg-white rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200 border border-[#e5e7eb] m-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-dashed border-[#e5e7eb] space-y-2">
                ...
                {paymentMethod === "QRIS" && (
                  <div className="flex items-start gap-2 text-xs bg-[#f9fafb] border border-[#e5e7eb] rounded-lg p-3">
                    <QrCode size={14} className="mt-0.5 text-[#111827]" />
                    <p className="text-[#6b7280] leading-relaxed">
                      Finalisasi setelah pelanggan membayar melalui QRIS luar sistem dan kasir mengecek pembayarannya.
                    </p>
                  </div>
                )}
              </div>
